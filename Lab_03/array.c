...
int bSearch(int *arr, int a, int b, int key) {
    ...
    // Initialize our value variable
    int i;

    // Initialize our midpoint variable
    int m = (a + b)/2;

    // If the top bound is larger than the bottom...
    if (b >= a){
        // We recursively check the following conditions to see what value the key is
        if (key == *(arr+m)){
            i = m;
        }
        if (key < *(arr+m)){
        ...
        // Otherwise the value does not exist and we return -1
        return -1;
    }
